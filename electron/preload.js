...
contextBridge.exposeInMainWorld('dino', {
  ...
  pairPart: (idA, idB) => ipcRenderer.invoke('parts:pair', idA, idB),
  removePart: (id) => ipcRenderer.invoke('parts:remove', id),
  listTrash: () => ipcRenderer.invoke('trash:list'),
  restorePart: (id) => ipcRenderer.invoke('trash:restore', id),
  gapSweep: (req) => ipcRenderer.invoke('nest:gapSweep', req),
  setSetFill: (id, fillSetId) => ipcRenderer.invoke('sets:setFill', id, fillSetId),
  listSets: () => ipcRenderer.invoke('sets:list'),
  createSet: (name) => ipcRenderer.invoke('sets:create', name),
  renameSet: (id, name) => ipcRenderer.invoke('sets:rename', id, name),
  removeSet: (id) => ipcRenderer.invoke('sets:remove', id),
  activateSet: (id) => ipcRenderer.invoke('sets:activate', id),
  setSetItem: (setId, partId, patch) => ipcRenderer.invoke('sets:setItem', setId, partId, patch),
